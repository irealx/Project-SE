...
st.set_page_config(page_title="ML Test", layout="centered")
...
st.title("Test — Machine Learning (Ensemble)")

st.markdown("""
<div class="card">
  <div style="font-size:1.1rem;font-weight:900;">ML Test Page</div>
  <div class="small-muted" style="margin-top:8px;">
    ทดสอบเฉพาะ <b>Ensemble (Voting)</b> และแสดง Probability ของ <b>UP</b>
  </div>
</div>
""", unsafe_allow_html=True)
...
btn = st.button("Predict (ML)", disabled=bool(errs))

if btn and not errs:
    X = scaler.transform(X_raw)
    proba_up = float(ensemble.predict_proba(X)[:, 1][0])
    ...
    st.caption("This page tests only the Ensemble model.")
    st.markdown("</div>", unsafe_allow_html=True)